...
# Generated by scripts/fetch_zoning_delay_data.py; import defensively so the
# seeder still works before the first fetch has run.
try:
    from app.data.ward_zoning_delay_data import ZONING_DELAY_META
except ImportError:  # pragma: no cover
    ZONING_DELAY_META: dict = {}  # type: ignore[no-redef]
...
async def import_scorecard_projects() -> None:
    ...
    for group_cfg in GROUP_CONFIG:
        ...
        for project_def in group_projects:
            ...
            # The delay data's frozen as-of date travels with the descriptors
            # so the scorecard can display the metric vintage.
            project_metrics_as_of: str | None = (
                str(ZONING_DELAY_META.get("computed_at"))
                if is_metrics_carrier and ZONING_DELAY_META.get("computed_at")
                else None
            )
            ...
            if existing_project:
                ...
                if (
                    ...
                    updated_config = DashboardConfig(
                        ...
                        metrics_as_of=project_metrics_as_of,
                    )

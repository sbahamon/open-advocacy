...
# Generated by scripts/fetch_affordability_data.py; import defensively so the
# scorecard seed still works before the module has ever been generated.
try:  # pragma: no cover - trivial import guard
    from app.data.ward_affordability_data import AFFORDABILITY_META
...
CHICAGO_WARD_METRICS: list[MetricDisplayConfig] = [
    ...
    # Community-collected Zillow affordability survey (hand-drawn ward
    # boundaries). Carries its own vintage/attribution via as_of/source.
    MetricDisplayConfig(
        key="affordable_share_pct",
        label="Affordable Listings (% at 60% AMI)",
        description=(
            "Share of the ward's Zillow rental and for-sale listings (July 2026) "
            "priced within the ARO 60% AMI limit for their bedroom count — rent "
            "for rentals, estimated monthly payment (no money down) for sales. "
            "Community-collected using hand-drawn ward boundaries that "
            "approximate the official 2023 ward map; small wards' percentages "
            "are noisy. One-week snapshot, not an annual average."
        ),
        format="percentage",
        show_in_table=True,
        show_in_tooltip=True,
        as_of=str(AFFORDABILITY_META.get("as_of") or "") or None,
        source=str(AFFORDABILITY_META.get("source") or "") or None,
    ),
    MetricDisplayConfig(
        key="affordability_rank_change",
        label="Affordability Rank Change (2025→2026)",
        description=(
            "How many places the ward moved in the 50-ward affordability ranking "
            "(by affordable-listings share) between the July 2025 and July 2026 "
            "surveys. Positive = the ward became relatively more affordable. "
            "Relative to other wards, so citywide price shifts cancel out."
        ),
        format="number",
        show_in_table=True,
        show_in_tooltip=True,
